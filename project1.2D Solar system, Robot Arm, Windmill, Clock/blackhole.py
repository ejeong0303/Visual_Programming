...
class Star:
    def __init__(self, startLoc, speed):
        ...
        self.reached_center = False # whether if it reached the center of the screen

    def update(self):
        # Gravitational pull
        direction = np.array([WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2, 1.0]) - self.loc
        # stars heading to the center of the screen
        distance = np.linalg.norm(direction) + 1e-10  # Adding a small constant to avoid division by zero
        # distance between the star and the center of the screen
        direction /= distance # normalizing vector
        pull = 500.0 / (distance ** 2 + 1e-10)
        # calculating gravity
        self.speed += pull
        self.loc += direction * self.speed
        # updates speed and calculates new location
        # Rotation
...
# create a list of 1000 Star objects with random starting locations and a speed of 0.1
